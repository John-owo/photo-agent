...
## 2026-09-19 - Community foundation and reproducible onboarding

- Isolated documentation work from public main 86d45d1 in codex/community-foundation-20260919; preserved dirty pending-validation checkout. Updated both README status summaries to match dated T09 evidence while retaining alpha and unvalidated batch boundaries. Added a synthetic first-run guide, FAQ, and workflow-feedback issue template. No runtime source changes.
- Fresh npm ci completed (npm reported an unapproved esbuild install-script warning; no approval configuration was changed). npm run build and npm run example passed: mock ACCEPTED, recovery REVIEW_REQUIRED, source_preserved=true, external_backend=false. Retained evidence under _agent_workspace/lightroom/verification/community-20260919. No live Lightroom or human gate was rerun.
- Initial sandbox GitHub reads failed due to network permissions; authorized network read/fetch succeeded. An initial guessed smoke-script read found no file; the actual examples/run-example.mjs was then inspected and run.
- Community research and private traffic baseline are kept outside the public repository. HN and r/opensource prohibit generated posts; no posts were submitted there. Reddit login is required before any r/mcp submission.
- Documentation verification passed: strict UTF-8, preserved existing CRLF, Markdown-only scope, relative file links, and git diff --check. New guide/template Prettier passed after fixing initial template formatting. No behavior code changed; build and executable onboarding example were the relevant checks.
